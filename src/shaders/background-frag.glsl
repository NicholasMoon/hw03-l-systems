...
float scene_SDF(out Intersection isect) {
  ...
  float fbm_val = fbm3D(isect.p + vec3((u_Time / 250.0f) + 225.0, 900, -575), 25.0, 0.025, 0.5, 6);
...
vec3 shadeIntersection(Ray r, Intersection isect) {
  ...
  if (isect.fbm_val <= 20.0f) {
    ...
    if (isect.fbm_val <= 23.0f) {
      float worley_val = worley3D(isect.p + vec3(u_Time / 250.0f, 200, 400), 3.0f);
      if (1.0f - worley_val >= 0.6f) {
        float worley_val_2 = worley3D(isect.p + vec3(u_Time / 250.0f, 200, 400), 0.185f);
...
vec3 shadeBackground(Ray r) {
  float worley_val = 1.0f - worley3D(r.d * 100.0f + vec3(u_Time / 250.0f, 200, 400), 1.0f);
  if (worley_val < 0.8f) {
    worley_val = 0.05;
  }
  else {
    worley_val = bias(worley_val, 0.005f) * 2.5f;
  }
  vec3 color = vec3(.9,1.1,1.5) * worley_val;
  return color;
}
